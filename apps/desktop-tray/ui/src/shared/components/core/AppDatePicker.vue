...
const calendarDays = computed(() => {
  ...
  return Array.from({ length: count }, (_, i) =>
    toDateString(viewYear.value, viewMonth.value, i + 1)
  );
});
...
const prevDisabled = computed(() => {
  ...
  return (
    toDateString(lastOfPrev.getFullYear(), lastOfPrev.getMonth(), lastOfPrev.getDate()) <
    props.minDate
  );
});
...
const triggerClass = computed(() => [
  ...
  props.disabled ? datePickerConfig.triggerStates.disabled : datePickerConfig.triggerStates.normal,
]);
...
<template>
  <div ref="rootEl" :class="datePickerConfig.wrapper">
    ...
    <div class="relative">
      <button type="button" :disabled="disabled" :class="triggerClass" @click="isOpen = !isOpen">
