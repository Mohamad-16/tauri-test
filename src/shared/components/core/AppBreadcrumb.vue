...
const visibleItems = computed<VisibleBreadcrumbItem[]>(() => {
  ...
  const first: VisibleBreadcrumbItem = { ...props.items[0]!, isCollapsedPlaceholder: false, index: 0 }
  const last: VisibleBreadcrumbItem = {
    ...props.items[props.items.length - 1]!,
    isCollapsedPlaceholder: false,
    index: props.items.length - 1,
  }
  const placeholder: VisibleBreadcrumbItem = { label: '...', isCollapsedPlaceholder: true, index: 1 }
  ...
  <nav :class="breadcrumbConfig.wrapper" :aria-label="t('core.breadcrumb.label')">
    <ol :class="breadcrumbConfig.list">
      <li
        ...
        :key="item.index"
        :class="breadcrumbConfig.item"
      >
        <div v-if="item.isCollapsedPlaceholder" :class="breadcrumbConfig.ellipsis">
          ...
        </div>
        <a
