  ...
  <div :class="uploadZoneConfig.wrapper">
    ...
    >
      ...
      <div v-if="processing" class="flex flex-col items-center gap-3 animate-fade-in py-4">
        ...
        <p :class="uploadZoneConfig.processingTitle">{{ t('core.upload.processing') }}</p>
        <p :class="uploadZoneConfig.processingHint">{{ t('core.upload.processingHint') }}</p>
      </div>

      <!-- Idle state -->
      <div v-else-if="selectedFiles.length === 0" class="flex flex-col items-center gap-3 py-4">
        <div :class="isDragOver ? uploadZoneConfig.iconWrap.dragOver : uploadZoneConfig.iconWrap.idle">
      ...
      <div v-else class="flex flex-col items-center gap-3 w-full">
        ...
        <div class="w-full max-w-xs space-y-2.5 mt-2">
          <div
            ...
            :key="file.name"
            :class="uploadZoneConfig.fileRow"
          >
            <FileText class="w-4 h-4 text-primary shrink-0" />
            <div class="flex-1 min-w-0">
              <p :class="uploadZoneConfig.fileName">{{ file.name }}</p>
              <p :class="uploadZoneConfig.fileSize">{{ formatSize(file.size) }}</p>
            </div>
          </div>
        </div>
      </div>
